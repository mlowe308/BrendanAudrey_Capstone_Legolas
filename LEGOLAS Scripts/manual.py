# 6/4/24 Inserted comments where Brendan and Audrey (BA) made a change.
...
# BA added import pdb and rpyc but I think core imports them anyway.
...
def check_for_miss(target_pos, current_pos):                       #inserted by BA
...
def goto_motor_pos(positions_map, entry_pos_name, motor):
    if motor is not None:
        ...
        print("moving device")                                  #inserted by BA
...
def goto_cell_loc(stage, entry_cell_loc):
    loc_ij = entry_cell_loc.get()                                #BA removed a print statement from original
    loc_i, loc_j = list( map(int, loc_ij.split(" ")) )
...
def error_target_missed_message():                #inserted by BA
