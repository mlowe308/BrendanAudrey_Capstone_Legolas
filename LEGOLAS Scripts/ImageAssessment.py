...
def plot_grid(x_coords, y_coords, horiz_lines, vert_lines, image):
...
def verify_x_pos(point, vert_line, error_space):
    ...
    vert_line_x = (point[1] - vert_line[1]) / vert_line[0]

    if point[0] < vert_line_x:
        x_offset = x_offset * (-1)
...
def verify_y_pos(point, horiz_line, error_space, diameter):
    ...
    horiz_line_y = horiz_line[0] * point[1] + horiz_line[1]

    if dist > (diameter/2):
        num_rows_away = dist // diameter
        horiz_line_y += diameter * num_rows_away

    if point[1] < horiz_line_y:
        dist = dist * (-1)

    y_offset = dist % diameter
    change_needed = (abs(y_offset) <= error_space)
    return change_needed, y_offset

def adjust_pos(point, horiz_lines, vert_lines, error, diameter):
    horiz_line, vert_line = get_nearest_line(point, horiz_lines, vert_lines)

    x_good, x_offset = verify_x_pos(point, vert_line, error)
    y_good, y_offset = verify_y_pos(point, horiz_line, error, diameter)
